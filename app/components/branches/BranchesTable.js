...
export default function BranchesTable({ selectedStoreId }) {
  ...
  // Use different endpoint based on whether store is selected
  ...
  const data = useMemo(() => {
    if (!branchesDataFinal?.data) return [];
    
    // Filter by search term if provided
    let filtered = branchesDataFinal.data;
    if (globalFilter && !selectedStoreId) {
      filtered = filtered.filter(
        (branch) =>
          branch.name?.toLowerCase().includes(globalFilter.toLowerCase()) ||
          branch.store?.name?.toLowerCase().includes(globalFilter.toLowerCase())
      );
    }
    ...
    return filtered.map((branch) => ({
      ...
      dateAdded: branch.created_at
        ? new Date(branch.created_at).toLocaleDateString("en-GB")
        : "---",
    }));
  }, [branchesDataFinal, globalFilter, selectedStoreId]);
      ...
      },
      {
        ...
        cell: (info) => (
          <div className="flex items-center gap-2 justify-center">
            ...
            <span className="text-gray-600 font-medium text-xs dir-ltr">
              {info.getValue()}
            </span>
          </div>
        ),
  ...
  const table = useReactTable({
    data,
    columns,
    state: { globalFilter: selectedStoreId ? "" : globalFilter },
    onGlobalFilterChange: setGlobalFilter,
    getCoreRowModel: getCoreRowModel(),
    getPaginationRowModel: getPaginationRowModel(),
    getFilteredRowModel: getFilteredRowModel(),
    enableGlobalFilter: !selectedStoreId,
  });
